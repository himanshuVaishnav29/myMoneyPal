...
const CurrWeekStatsByTagBar = () => {
    ...
    useEffect(() => {
        if (data?.getCurrentWeekStatsByTag) {
            const tags = data.getCurrentWeekStatsByTag.map(stat => stat.tag);
            const amounts = data.getCurrentWeekStatsByTag.map(stat => stat.totalAmount);
...
export default CurrWeekStatsByTagBar;