...
const CurrMonthStatsByTag = () => {
    ...
    useEffect(() => {
        if (data?.getCurrentMonthStatsByTag) {
            const tags = data.getCurrentMonthStatsByTag.map((stat) =>
                stat.tag.charAt(0).toUpperCase() + stat.tag.slice(1)
            );
            const totalAmounts = data.getCurrentMonthStatsByTag.map((stat) => stat.totalAmount);
...
export default CurrMonthStatsByTag;
